...
<label class="col-sm-3 col-form-label" for="{{ $input['id'] ?? $input['name'] }}" data-browse="Browse">{{ $input['placeholder'] ?? ''}}</label>
</div>
@if(isset($input['helpText']))
  <small class="form-text text-muted">{{ ($input['helpText']) ?? '' }}</small>
@endif
@if(isset($input['error']))<div class="invalid-feedback">{{ ($input['error']) }}</div>@endif
